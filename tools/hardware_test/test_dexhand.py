...
from pyzlg_dexhand.dexhand_interface import (
    LeftDexHand, RightDexHand, ControlMode, ZCANWrapper, HandFeedback
)
...
class DexHandTester:
    ...
    def __init__(self, hand_names: List[str], log_dir: str = "dexhand_logs",
                 settling_time: float = 1.0, n_samples: int = 5):
    ...
    def _collect_steady_state(self, target_angles: Dict[str, float],
                            settling_time: Optional[float] = None) -> List[HandFeedback]:
    ...
    def _compute_steady_state_metrics(self, joint_name: str,
                                    target_angle: float,
                                    samples: List[HandFeedback]) -> Tuple[float, float]:
    ...
    def _test_reset(self):
        ...
        for joint in JOINT_CONFIGS:
            ...
            self.results['individual'][joint].append(TestResult(
    ...
    def _test_individual_joints(self):
        ...
        for joint_name, config in JOINT_CONFIGS.items():
            ...
            try:
                ...
                self.results['individual'][joint_name].append(TestResult(
                    timestamp=start_time,
    ...
    def _test_simultaneous(self):
        ...
        try:
            ...
            for joint_name in JOINT_CONFIGS:
                ...
                logger.info(f"{joint_name}: Target={joint_commands[joint_name]:.1f}°, "
            ...
            for joint_name in JOINT_CONFIGS:
                ...
                logger.info(f"{joint_name}: Target=0.0°, "
                        f"Steady-state={ss_angle:.1f}°, Error={ss_error:.1f}°")
        ...
        with open(report_path, "w") as f:
            ...
            for joint_name, results in self.results['individual'].items():
                ...
                for i, result in enumerate(results):
                    ...
                    f.write(f"    Steady-state angle: {result.steady_state_angle:.2f}°\n")
                    f.write(f"    Steady-state error: {result.steady_state_error:.2f}°\n")
            ...
            for result in self.results['simultaneous']:
                joint_name = next(name for name in JOINT_CONFIGS
                                if abs(result.command_angle - 30.0) < 0.1 or
                                abs(result.command_angle) < 0.1)
            ...
            for joint_name, results in joint_results.items():
                ...
                f.write(f"    Steady-state angle: {results[0].steady_state_angle:.2f}°\n")
                f.write(f"    Steady-state error: {results[0].steady_state_error:.2f}°\n")
                f.write(f"    Settling time: {results[0].settling_time:.2f}s\n")

                # Return movement
                f.write("  Return Movement:\n")
                f.write(f"    Command: {results[1].command_angle:.1f}°\n")
                f.write(f"    Steady-state angle: {results[1].steady_state_angle:.2f}°\n")
                f.write(f"    Steady-state error: {results[1].steady_state_error:.2f}°\n")
            ...
            if feedback_data:
                ...
                for joint in joints:
                    ...
                    f.write(f"    RMS error: {np.sqrt(np.mean(np.array(up_errors)**2)):.2f}°\n")
                    ...
                    f.write(f"    RMS error: {np.sqrt(np.mean(np.array(down_errors)**2)):.2f}°\n")
            ...
            for joint_name in JOINT_CONFIGS:
                ...
                ind_errors = [r.steady_state_error for r in self.results['individual'][joint_name]]
                ...
                f.write(f"    Forward Error: {sim_results[0].steady_state_error:.2f}°\n")
    ...
    def _plot_steady_state_errors(self):
        ...
        for joint in joints:
            ...
            max_result = [r for r in self.results['individual'][joint]
                         if r.command_angle > 0][0]
    ...
    def _plot_consecutive_movement(self):
        ...
        plt.plot(commands_df['time'], commands_df['angle'], 'k--',
        ...
        for joint in joint_cols:
            ...
            plt.plot(feedback_df['time'], angles, '.-', label=f'{joint}',
        ...
        for joint in joint_cols:
            ...
            cmd_interp = np.interp(feedback_df['time'],
                                 commands_df['time'],
                                 commands_df['angle'])
            error = angles - cmd_interp
            plt.plot(feedback_df['time'], error, '.-', label=f'{joint}',
                    alpha=0.7, markersize=2)
    ...
    def _plot_simultaneous_results(self):
        ...
        forward_errors = [abs(results[0].steady_state_error)
                        for results in joint_results.values()]
        return_errors = [abs(results[1].steady_state_error)
                        for results in joint_results.values()]
...
def main():
    ...
    parser.add_argument("--hands", nargs="+", choices=['left', 'right'],
                      default=['left'], help="Which hands to test")
    ...
    try:
        ...
        tester = DexHandTester(args.hands, args.log_dir,
                             args.settling_time, args.n_samples)
        tester.run_tests()
